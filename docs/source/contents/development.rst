...
.. code-block:: shell
    ...
    git clone git@github.com:alan-techdev/rf-metadata-displayer.git
    cd  rf-metadata-displayer
...
4. **CI(Continuous Integration)/CD(Continuous Delivery) Pipeline**, running on `Github Actions`, defined in `.github/`
   ...
   b. **Continuous Deployment**
      ...
      `Production`
         ...
         1. **Python Distribution** to `pypi.org`_, on `tags` **v***, pushed to `main` branch
         2. **Docker Image** to `Dockerhub`_, on every push, with automatic `Image Tagging`
      
      `Staging`

         3. **Python Distribution** to `test.pypi.org`_, on "pre-release" `tags` **v*-rc**, pushed to `release` branch
